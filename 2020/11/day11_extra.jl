...
function shag1!(b1, b2)
    w, h = size(b1)
    @inbounds for i in 2:w - 1, j in 2:h - 1
...
@btime part("input.txt", $shag1!)
b1, b2 = load_data("input.txt")
@btime part_internal(cb1, cb2) setup=(cb1=deepcopy($b1), cb2=deepcopy($b2))

@btime part_internal($b1) 

println("Part 1: ", part("input_test2.txt", shag1!))
