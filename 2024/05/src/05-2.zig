...
fn normalizeLine(items: []u32, graph: *const Graph) void {
    std.mem.sort(u32, items, graph, struct {
        fn lessThan(g: *const Graph, m1: u32, m2: u32) bool {
            return g.contains(m1, m2);
        }
    }.lessThan);
}

fn calculateLine(items: []u32) u32 {
...
fn analyzePages(reader: Reader, graph: *const Graph) !u32 {
    ...
    while (try reader.readUntilDelimiterOrEof(&buffer, '\n')) |line| {
        ...
        if (validateLine(items, graph)) continue;
        // print("original: {any}, ", .{items});
        normalizeLine(items, graph);
        // print("sorted: {any}\n", .{items});

        result += calculateLine(items);
