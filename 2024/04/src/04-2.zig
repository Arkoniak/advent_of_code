...
fn readWord(buffer: *[5]u8, m: *Matrix, p0: P) bool {
    for (DIRS, 0..5) |dir, i| {
        const p = p0.move(dir);
...
pub fn main() !void {
    ...
    while (it.next()) |p| {
        if (readWord(&buf, &matrix, p)) {
            // print("Position: ({d},{d}), word: {s}\n", .{ p.r, p.c, buf });
            if (std.mem.eql(u8, &buf, "MSAMS")) result += 1;
            if (std.mem.eql(u8, &buf, "MMASS")) result += 1;
            if (std.mem.eql(u8, &buf, "SMASM")) result += 1;
            if (std.mem.eql(u8, &buf, "SSAMM")) result += 1;
            // if (std.mem.eql(u8, &buf, "SMAMS")) result += 1;
            // if (std.mem.eql(u8, &buf, "MSASM")) result += 1;
        }
    }
