...
pub fn validate(result: u64, acc: u64, data: []const u64) bool {
    ...
    return validate(result, acc * numlen(data[0]) + data[0], data[1..]);
}
...
test "verify number length" {
    try testing.expect(numlen(123) == 1000);
    try testing.expect(numlen(12) == 100);
    try testing.expect(numlen(1234) == 10000);
}
