...
const V = struct {
    ...
    pub fn scale(self: V, lambda: i16) V {
        return .{ .r = self.r * lambda, .c = self.c * lambda };
    }
};
...
const Puzzle = struct {
    ...
    fn gen_point(self: *const Self, p1: P, p2: P) ?P {
        const p = p2.move(p1.vec(p2));
        // print("P1: {}, P2: {}, P: {}\n", .{ p1, p2, p });
    ...
    fn items_count(self: *const Self, items: []P, set: *Set) !void {
        ...
        for (0..items.len) |i| {
            ...
            while (j < items.len) {
                const p2 = items[j];
                const pa = self.gen_point(p1, p2);
                if (pa != null) {
                    try set.put(pa.?, {});
                }
                const pb = self.gen_point(p2, p1);
                if (pb != null) {
                    try set.put(pb.?, {});
                }
